...
if f1=="" or canopus=="" or logn=="":
    ...
    while line1:
        ...
        if line1.startswith('#') or line1.startswith('id'):
            pass
        else:
            m+=1
            line1=line1.replace('"','')
            ...
            #changes made here \/ to use canopus_compound_summary.tsv rather than old format

            #xid=tab1[0]; form=tab1[1]; adduct=tab1[2]; mainclasses=tab1[4:9]; altclasses=tab1[9]
            xid=tab1[0]; form=tab1[1]; adduct=tab1[2]; mainclasses=[tab1[i] for i in (10,12,14,16,18)]; altclasses=tab1[20]
            ...
            if len(annot)>=3: 
                ...
                for cpd in dict1: #PlantCyc compounds
                    ...
                    if abs(y-len(annot))<=3: #At least three good matches                        
                        ...
                        #Calculate Tanimoto based on all annotations, same as Jaccard       <-this is the test block of code

                        #try annot2 = annot + annot1
                        annot2 = annot + annot1

                        numerator = len(set(annot2) & set(chemont))
                        denominator = len(annot2)+len(chemont)-numerator
                        overlap=numerator/min([len(annot2), len(chemont)])
                        tcoeff = numerator/denominator                        
                        tanimoto = '{:2f}'.format(tcoeff)

                        #Calculate Tanimoto based on all annotations, same as Jaccard       <-this is the original block of code
